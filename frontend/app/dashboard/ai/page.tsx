...
import { Sparkles, Wand2, Brain, Star, CheckCircle, Loader2, Eye, EyeOff, Trash2, ArrowUpRight, Zap } from 'lucide-react';
import { useAuthStore, DEMO_MEDIA } from '@/lib/auth-store';
import toast from 'react-hot-toast';

// Simulate AI scoring of images
function scoreImage(media: any, index: number) {
    // Deterministic pseudo-scoring for demo
    const scores: Record<string, number> = {
        'm1': 94, 'm2': 87, 'm3': 91, 'm4': 78,
        'm5': 72, 'm6': 89, 'm7': 85, 'm8': 96,
        'm9': 61, 'm10': 82, 'm11': 88, 'm12': 93,
    };
    return scores[media.id] ?? Math.floor(60 + Math.random() * 35);
}
...
export default function AIToolsPage() {
    ...
    const runAI = async () => {
        ...
        const withScores = DEMO_MEDIA
            .filter(m => m.type === 'photo')
            .map((m, i) => ({ media: m, score: scoreImage(m, i) }))
    ...
    return (
        <div style={{ padding: '40px', maxWidth: 1100 }}>
            ...
            {/* Run AI panel */}
            <div style={{
                background: done ? 'var(--surface)' : 'linear-gradient(135deg, rgba(201,151,74,0.06) 0%, var(--surface) 100%)',
                border: `1px solid ${done ? 'var(--border-subtle)' : 'var(--border-accent)'}`,
                borderRadius: 'var(--radius-xl)', padding: 36, marginBottom: 32,
            }}>
                {!running && !done && (
                    <div style={{ textAlign: 'center' }}>
                        ...
                        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)', marginBottom: 28, maxWidth: 440, margin: '0 auto 28px', lineHeight: 1.7 }}>
                            Scan all {DEMO_MEDIA.filter(m => m.type === 'photo').length} photos and automatically select the best shots
                            to create your &ldquo;Top Highlights&rdquo; album. Powered by computer vision.
                        </p>
                        <button onClick={runAI} className="btn btn-primary" style={{ padding: '12px 32px', borderRadius: 'var(--radius-md)', fontSize: 'var(--text-md)' }}>
                            <Sparkles size={16} />
                            Generate Highlights
                        </button>
                        {isDemoMode && (
                            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginTop: 12 }}>
                                ⚡ Demo mode — uses built-in scoring algorithm
                            </div>
                        )}
                    </div>
                )}
                ...
                {done && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
                        <div style={{
                            ...
                            background: 'rgba(80,160,100,0.15)', border: '1px solid rgba(80,160,100,0.3)',
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>
                                Scored {scored.length} photos · Average score: {Math.round(scored.reduce((s, x) => s + x.score, 0) / scored.length)}/100
                            </div>
                        </div>
                        <div style={{ display: 'flex', gap: 10 }}>
                            ...
                            <button onClick={() => toast.success('Highlights album saved!')} className="btn btn-primary" style={{ padding: '8px 16px', fontSize: 'var(--text-sm)' }}>
                                <Star size={13} />
                            ...
                            return (
                                <motion.div key={m.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.03 }}>
                                    <div style={{ position: 'relative', borderRadius: 'var(--radius-md)', overflow: 'hidden', border: `1px solid ${isHighlight ? 'var(--border-accent)' : 'var(--border-subtle)'}` }}>
                                        <img src={m.thumbnailUrl || m.url} alt="" style={{ width: '100%', aspectRatio: '1', objectFit: 'cover', display: 'block' }} loading="lazy" />

                                        {/* Score badge */}
                                        <div style={{
                                            position: 'absolute', top: 8, left: 8,
                                            background: isHighlight ? 'linear-gradient(135deg, var(--gold-dark), var(--gold))' : 'rgba(0,0,0,0.7)',
                                            color: isHighlight ? 'var(--obsidian)' : '#fff',
            ...
            <div style={{ marginTop: 48 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
                    {[
                        ...
                        { icon: '🎬', title: 'Auto Video Highlights', desc: 'Extract best moments from your Vimeo film and generate a 30-sec reel.', plan: 'Pro' },
                        ...
                        <div key={i} style={{
                            ...
                            <div style={{
                                ...
                                background: f.plan === 'Studio' ? 'var(--surface-2)' : 'var(--surface-frost)',
                                border: `1px solid ${f.plan === 'Studio' ? 'var(--border-muted)' : 'var(--border-accent)'}`,
                                color: f.plan === 'Studio' ? 'var(--text-muted)' : 'var(--gold)',
                                fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 4, letterSpacing: '0.05em',
                            }}>
                                {f.plan}
                            </div>
